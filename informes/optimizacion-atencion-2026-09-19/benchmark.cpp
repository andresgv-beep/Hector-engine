// Same binary, model and enabled CUDA graphs; only split attention changes.
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::fprintf(stderr, "Usage: benchmark model.hnf split(0|1)\n");
        return 2;
    }
    ...
    cfg.use_split_attention = std::atoi(argv[2]) != 0;
    ...
    for (int repeat : {0, 128, 384}) {
        ...
        for (int trial = 0; trial < trials; ++trial) {
            ...
            std::printf("BENCH split=%d repeat=%d trial=%d prompt=%u output=%u prefill_ms=%.3f decode_ms=%.3f tok_s=%.3f used_MiB=%.1f captures=%u replays=%u fallbacks=%u hash=%llu bytes=%zu\n",
                cfg.use_split_attention, repeat, trial, stats.prefill_tokens,
            ...
            if (const char* directory = std::getenv("ATTENTION_OUTPUT_DIR")) {
                const auto path = std::string(directory) + "/split-" + argv[2] + "-" + std::to_string(repeat) + "-" + std::to_string(trial) + ".txt";
