...
for mode in (0, 1):
    rows = []
    for line in (root / f"graphs-{mode}.log").read_text().splitlines():
        if line.startswith("BENCH "):
            rows.append(dict(field.split("=", 1) for field in line.split()[1:]))
    assert len(rows) == 18, f"Incomplete benchmark: graphs={mode}"
    records[mode] = rows
...
for repeat in (0, 128, 384):
    ...
    assert all(int(r["captures"]) == 0 and int(r["replays"]) == 0 for r in group[0])
    row = {"prompt_tokens": int(group[0][0]["prompt"]), "samples_per_mode": 5}
    for m, label in ((0, "eager"), (1, "graphs")):
    ...
    row["gain_percent"] = (row["graphs"]["median_tok_s"] / row["eager"]["median_tok_s"] - 1) * 100
