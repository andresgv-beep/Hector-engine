...
main = framed("Explica en dos frases cómo funciona una base de datos relacional.")
...
with tempfile.TemporaryDirectory(prefix="hector-bridge-") as home:
    result = subprocess.run(
        [str(repo / "build/helios_formatted"), sys.argv[1], "4096", "<turn|>"],
...
assert int(fields[6]) > 0, "Repeated main request did not reuse its prefix"
